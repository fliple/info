...
async function getBulkPairData(pairList, ethPrice) {
  ...
  let [{ number: b1 }, { number: b2 }] = await getBlocksFromTimestamps([t1, t2, tWeek])
...
export function useHourlyRateData(pairAddress, timeWindow) {
  ...
  useEffect(() => {
    ...
    const startTime =
      timeWindow === timeframeOptions.ALL_TIME ? 1628319889 : currentTime.subtract(1, windowSize).startOf('hour').unix()
